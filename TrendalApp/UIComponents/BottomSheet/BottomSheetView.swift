...
public struct BottomSheetView<Content>: DismissableView where Content: View {
        ...
        GeometryReader { geometry in
            VStack(spacing: 0) {
                ...
                content
                    ...
                    .onTapGesture {}
                Rectangle().fill(.clear).frame(maxWidth: geometry.size.width, maxHeight: BottomSheetConstants.bottomPaddingHeight)
            }
            .ignoresSafeArea()
            .readSize {
    ...
    private var maxOffset: CGFloat {
        ...
        return offset + BottomSheetConstants.bottomPaddingHeight + max(translation, -configuration.maxOverDrag)
    }

    private func dismissIfNeededOnDragEnd(translationHeight: CGFloat) {
        let ratioHeight = (contentHeight - BottomSheetConstants.bottomPaddingHeight) * configuration.dismissRatio
